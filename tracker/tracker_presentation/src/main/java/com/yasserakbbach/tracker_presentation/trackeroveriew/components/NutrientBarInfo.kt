...
fun NutrientBarInfo(
    ...
    val defaultOrExceedColor = if(value <= goal) MaterialTheme.colorScheme.onPrimary else goalExceededColor
    ...
    LaunchedEffect(key1 = value) {
        angleRatio.animateTo(
            targetValue = if (goal > 0) value.div(goal.toFloat()) else 0F,
            animationSpec = tween(
                durationMillis = 300,
            )
        )
    ...
    Box(
        ...
        Column(
            ...
            UnitDisplay(
                ...
                amountColor = defaultOrExceedColor,
                unitColor = defaultOrExceedColor,
            )
            Text(
                text = name,
