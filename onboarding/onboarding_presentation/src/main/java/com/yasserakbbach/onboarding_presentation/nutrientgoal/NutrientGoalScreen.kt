    ...
    Scaffold(
        ...
        Box(
            ...
            Column(
                ...
                UnitTextField(
                    value = viewModel.nutrientGoalState.carbsRatio,
                    onValueChange = { viewModel.onEvent(NutrientGoalEvent.OnCarbsRatioChange(it)) },
                    unit = stringResource(id = R.string.percent_carbs),
                )
                Spacer(modifier = Modifier.height(spacing.spaceSmall))
                UnitTextField(
                    value = viewModel.nutrientGoalState.proteinRatio,
                    onValueChange = { viewModel.onEvent(NutrientGoalEvent.OnProteinsRatioChange(it)) },
                    unit = stringResource(id = R.string.percent_proteins),
                )
                Spacer(modifier = Modifier.height(spacing.spaceSmall))
                UnitTextField(
                    value = viewModel.nutrientGoalState.fatRatio,
                    onValueChange = { viewModel.onEvent(NutrientGoalEvent.OnFatsRatioChange(it)) },
